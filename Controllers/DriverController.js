...
const login = async (payloadData) => { //console.log("login====init");
  let userData;
  ...
  try {
    ...
    let getData = await Service.DriverService.getData(criteria,{},{ lean: true });
    if (getData.length == 0) {
      throw STATUS_MSG.ERROR.INVALID_EMAIL_PASSWORD;
    }
    if (getData[0].password !=UniversalFunctions.encryptedPassword(payloadData.password)) {
      throw STATUS_MSG.ERROR.INVALID_EMAIL_PASSWORD;
    }
    let userData = getData[0];  
    let checkDevice = await UniversalFunctions.checkDeviceTokenAndDelete(deviceData,APP_CONSTANTS.USER_ROLES.DRIVER);
    ...
    let finalData = await Service.DriverService.updateData(updateCriteria,
      dataToSet,{ new: true, lean: true });
...
let verifyOtp = async (payloadData) => {
  ...
  try {
    let criteria = { phoneNo: payloadData.mobileNumber};
    let userData = await Service.DriverService.getData(criteria,{},{lean:true});
    if (userData.length == 0) {
      throw STATUS_MSG.ERROR.NOT_FOUND;
    }
    ...
    let updatePassWord = await Service.DriverService.updateData(criteria,dataToSet,optionsU);
    delete updatePassWord.password;
    delete updatePassWord.__v;
    if (payloadData.isForgotPassword) {
...
let resetPassword = async (payloadData) => {
  ...
  try {
    ...
    let updatePassWord = await Service.DriverService.updateData(criteria,dataToSet,{new:true});
    return {};
  } catch (err) {
    throw err;
  }
...
let uploadDocument = async (payloadData, UserData) => {
  ...
  let docPreFix = (payloadData.documentType== 1) ? "doc_adharcard_" : (payloadData.documentType==2) ? "doc_drivingLicense_" : "doc_other_"; 
  try {
    let criteria = { _id: UserData._id };
    if (typeof payloadData.document == "undefined") {
      throw STATUS_MSG.ERROR.INVALID_FILE;
    ...
    driverProfilePic = await Service.DriverService.updateMultipleDocuments(criteria,dataToSet,{lean:true});
    return {};
  } catch (err) {
    //console.log("err",err);
    throw err;
...
let getDashboardData = async(payloadData, UserData) => {
  try {
    let tOrderCriteria = {driverId:UserData._id,status:APP_CONSTANTS.ORDER_STATUS.DELIVERED_BY_RIDER};
    let nOrderCriteria = {driverId:UserData._id,
      status:{
    ...
    let cOrderCriteria = {driverId:UserData._id,
...
let driverOrderUpdateStatus = async (payload,UserData)=>{ 
  let collectionOptions =[
    ...
    }, 
    {
      path: 'customerId',
      model: 'customer',
      select: '_id firstName rating lastName countryCode mobileNumber deviceType deviceToken',
      options: {lean: true}
    },   
  ];
  ...
  try{
    let queryResult= await Promise.all([
      Service.DAOService.getDataWithReferenceFixed(Models.OrderTable,criteriaOrder,{},{lean:true},collectionOptions),
    ...
    if(orderData[0].driverId!=UserData._id) throw STATUS_MSG.ERROR.INVALID_ORDER_ID;
    ...
    if(payload.status==APP_CONSTANTS.ORDER_STATUS.PICKED_BY_RIDER){
     let getMessage = '';
      ...
      if(orderData[0].status==APP_CONSTANTS.ORDER_STATUS.RIDER_REACHED_LOCATION || orderData[0].status==APP_CONSTANTS.ORDER_STATUS.DELIVERED_BY_RIDER){
        throw STATUS_MSG.ERROR.YOU_CAN_NOT_PERFORM_THIS_ACTION;
      }
    }else if(payload.status==APP_CONSTANTS.ORDER_STATUS.RIDER_REACHED_LOCATION){
      let getMessage = '';
      notificationData.forEach(element => {
        if(element.event == APP_CONSTANTS.ORDER_STATUS.RIDER_REACHED_LOCATION){
          getMessage = element.event_message;
        }
      });
      ...
      messageNotification=UserData.fullName ` ${getMessage}`;
      if(orderData[0].status==APP_CONSTANTS.ORDER_STATUS.RIDER_REACHED_LOCATION){
        throw STATUS_MSG.ERROR.YOU_ALREADY_REACHED_CUSTOMER_LOCATION;
      }
    }else if(payload.status==APP_CONSTANTS.ORDER_STATUS.DELIVERED_BY_RIDER){
      let getMessage = '';
      notificationData.forEach(element => {
      ...
      messageNotification=UserData.fullName ` ${getMessage}`;
      if(orderData[0].status==APP_CONSTANTS.ORDER_STATUS.DELIVERED_BY_RIDER){
    ...
    if(payload.status==APP_CONSTANTS.ORDER_STATUS.DELIVERED_BY_RIDER){
      ...
      let orderResult = await Promise.all([
        ...
        Service.OrderService.updateMultipleDocuments(criteriaOrder,dataToSet,{lean:true}),
        Service.TransactionService.InsertData(transctionData),
        Service.RestaurantService.updateMultipleDocuments({_id:orderData[0].restaurantId},restaurantBalanceUpdate,{lean:true}),
        Service.DriverService.updateMultipleDocuments({_id:UserData._id},driverBalanceUpdate,{lean:true}),
      ]);
    }else{
      await Service.OrderService.updateMultipleDocuments(criteria,dataToSet,options);
    }    
...
let addWalletbalance = async (payloadData, UserData) => {
  try { 
    ...
    let captureRazorpayPayment = await RazorpayPaymentsController.capturePayment({transaction_id:payloadData.transactionId,
    ...
    if(captureRazorpayPayment.status=="success"){
      ...
      let updateWalletbalance = await Service.DriverService.updateData(criteria,dataToSet,{new:true});
      return {};
    }else{
...
let orderRequestAcceptedAndRejected = async (payload,UserData)=>{ 
  try{
    ...
    let criteria2 = {orderId: payload.orderId,status:APP_CONSTANTS.ORDER_STATUS.ACCEPTED_BY_DRIVER}
    let options = {lean: true};    
    let dataToSet;
    if(payload.orderRequestStatus == APP_CONSTANTS.ORDER_STATUS.ACCEPTED_BY_DRIVER){
      let checkOrderRequest= await Service.DriverOrderRequestService.getData(criteria2,{},options);
      if(checkOrderRequest.length>0){
        throw STATUS_MSG.ERROR.ORDER_ALREADY_ACCEPTED; 
      }
      dataToSet = {status:payload.orderRequestStatus,acceptedDate:Date.now()};
    }else if (APP_CONSTANTS.ORDER_STATUS.REJECTED_BY_DRIVER) {
      dataToSet = {status:payload.orderRequestStatus,rejectedDate:Date.now()};
    }
    let queryResult= await Promise.all([
      Service.DriverOrderRequestService.updateMultipleDocuments(criteria,dataToSet,options),
    ]);
    if(payload.orderRequestStatus == APP_CONSTANTS.ORDER_STATUS.ACCEPTED_BY_DRIVER){
      await Service.OrderService.updateMultipleDocuments({_id:payload.orderId},{isdriverAssigned:true,driverId:UserData._id},{})
    }
    return {};
  }catch(err){
    throw err;
    ...
    let collectionOptions =[
      {
        path: 'orderId',
        model: 'orderTables',
        select: '_id totalPrice address paymentType restaurantId orderAuoIncrement',
        options: {lean: true}
      }
    ]
    let nestedModel = [
      {
        ...
        model: 'restaurant',
        select: '_id restaurantName logo rating vendorFullAddress restaurantAutoIncrementId',
        options: {lean: true}
      }, 
    ];
    let queryResult= await Promise.all([
      Service.DriverOrderRequestService.getData(criteria,{},options),
      Service.DAOService.getDataDeepPopulateFixed(Models.DriverOrderRequestTable,criteria,{__v:0},options,collectionOptions,nestedModel)
    ])
...
let getBonusData = async(payloadData, UserData) => {
  ...
  try { 
    let criteria={isDeleted:false},options={lean:true}
    let queryResult= await Promise.all([
  ...
  try { 
    ...
    let dataToSet = {
     ...
     let withdrawTransaction = {
       driverId : UserData._id,
       transactionType: APP_CONSTANTS.TRANSACTION_TYPES.SUBMIT_WITHDRAW_FROM_DRIVER,
...
let getNotificationList = async (payload,UserData)=>{ //console.log("UserData",UserData); 
  try{
    let  criteria = {userId: UserData._id,userType:APP_CONSTANTS.USER_ROLES.DRIVER}; 
