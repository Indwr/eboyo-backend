...
const registration  = async (payloadData)=> { 
  ...
  try { 
    ...
    let checkUserExistOrNot= await Service.CustomerService.getData(criteria, {},{lean:true}); 
    ...
    let checkDevice =  await  UniversalFunctions.checkDeviceTokenAndDelete(deviceData,APP_CONSTANTS.USER_ROLES.CUSTOMER);
    ...
    UniversalFunctions.sendSMS(payloadData.mobileNumber,otp,LANGUAGE_SPECIFIC_MESSAGE.verificationCodeMsg.EN+otp);
...
let updateProfile = async (payload,UserData)=>{ //console.log("UserData",UserData); 
  try{
...
let uploadProfilePic = async (payloadData, UserData) => {
  try {
    ...
    let project= {accessToken:0,__v:0,deviceType:0,password:0,otpCode:0,isDeleted:0,isBlocked:0,passwordResetToken:0,emailVerified:0,mobileVerified:0}
    driverProfilePic = await Service.CustomerService.updateMultipleDocuments(criteria,dataToSet,{lean: true});
...
let verifyOtp = async (payloadData)=> { //
  try{
   ...
   let userData = await Service.CustomerService.getData(criteria,{},{lean:true}); 
    ...
    let updatePassWord    = await Service.CustomerService.updateData(criteria,dataToSet,optionsU);    
    delete updatePassWord.password;
    delete updatePassWord.__v;
    if(payloadData.isForgotPassword){
...
let addAdress = async (payloadData,UserData)=>{ //console.log("UserData",UserData); 
  try{     
    ...
    if(payloadData.addressId && payloadData.addressId!=""){
      ...
      let addressData  = await Service.CustomerAddressService.updateData(criteria,dataToSet,{new:true});
      return {addressData};
    }else{
      let addressData  = await Service.CustomerAddressService.InsertData(dataToSet);
      return {addressData};
    }
...
let getAddress = async (payload,UserData)=>{ //console.log("UserData",UserData); 
  try{
...
let getDefaultAddress = async (payload,UserData)=>{ //console.log("UserData",UserData); 
  try{
    let  criteria = {customerId: UserData._id,isDefault:true }; 
    let projection = {__v: 0};
     let defautlAddress = await Service.CustomerAddressService.getData(criteria,projection,{});
    return {
...
let setDefaultAddress = async (payloadData,UserData)=>{ 
  ...
  try {
    let addressData= await Promise.all([
      Service.CustomerAddressService.getData(criteria, projection, {lean: true}),
      Service.CustomerAddressService.getData(criteria1, projection, {lean: true}),
    ]);
    ...
    if(updateLastDefaultAddress){
      ...
      let dataToSet1 = {isDefault:false,updatedAt: new Date()};
       finalData = await Promise.all([
         Service.CustomerAddressService.updateMultipleDocuments(updateCriteria1,dataToSet1,{ multi: true }),
         Service.CustomerAddressService.updateData(updateCriteria,dataToSet,{ new: true }),
      ]);
      finalData = finalData[1];
    }else{
      finalData = await Service.CustomerAddressService.updateData(updateCriteria,dataToSet,{ new: true });
    }
...
let deleteAddress = async (payloadData,UserData)=>{ 
  ...
  let projection = {__v: 0};
  try {
    let addressData= await  Service.CustomerAddressService.getData(criteria, projection, {lean: true});
    ...
    await Service.CustomerAddressService.updateData(criteria,dataToSet,{new:true});
    return {};
  } catch (err) {
    throw err;
  }
...
let calculateCartCharges = async (payloadData,UserData)=>{
  ...
  let promoCodeIsValidOrNot=true,finalDistance=0,deliveryCharge=0;
  let promoCodeDiscount=0,totalPriceAfterDiscount=0;
  ...
  try{
    ...
    let restaurantData = await Service.RestaurantService.getData(RestaurantCriteria,{__v:0,password:0,accessToken:0,deliveryServiceArea:0}, options);
    restaurantData = JSON.parse(JSON.stringify(restaurantData)); 

    let distanceResult = await Helper.OrderHelper.calculateDeliveryCharge(restaurantData[0].cityId,restaurantData[0].location,payloadData.customerLat,payloadData.customerLong);

    //PromoCodeData
     
         
    let restaurantGstPercentage = (restaurantData.length>0 && ("restaurantGstPercentage" in restaurantData[0]))? restaurantData[0].restaurantGstPercentage:0;
    
    //Calculate Restaurnt Gst
    let restaurantCalculateGst = 0 ,totalPriceWithGst=0;
    if(restaurantData.length>0 && ("restaurantGstPercentage" in restaurantData[0]) && restaurantData[0].restaurantGstActivated){
      restaurantCalculateGst= (payloadData.totalPrice*restaurantData[0].restaurantGstPercentage)/100;
    ...
    if(payloadData.promoCode){
      PromoCodeData= await Helper.OrderHelper.checkPromoCode(payloadData.promoCode,payloadData.restaurantId,UserData._id,0,payloadData.totalPrice,distanceResult.customerDeliveryCharge,restaurantCalculateGst);
    ...
    let finalBillAmount= payloadData.totalPrice+distanceResult.customerDeliveryCharge+restaurantCalculateGst-discountAmount;
    finalDistance=distanceResult.finalDistance;
    console.log("finalBillAmount",finalBillAmount,payloadData.totalPrice,"\ncustomerDeliveryCharge:",distanceResult.customerDeliveryCharge,"\nrestaurantCalculateGst",restaurantCalculateGst,"\ndiscountAmount:",discountAmount);
    return {
...
let submitDriverRating = async (payloadData,customerData,orderData)=>{
  try{
    ...
    let dataToSet = {"carrierRatingByCustomer.ratingGivenAt" : new Date(),"carrierRatingByCustomer.rating":payloadData.rating,"carrierRatingByCustomer.comment":payloadData.comment ,carrierRating:true};
    ...
    let calculateAvarageRating = (driverDetails[0].rating.averageRating != 0) ? (driverDetails[0].rating.totalRating+payloadData.rating)/(driverDetails[0].rating.noOfTimesRated+1): payloadData.rating;    
    let ratingDataToSet = {$inc:{"rating.totalRating":payloadData.rating,"rating.noOfTimesRated":1},"rating.averageRating":calculateAvarageRating.toFixed(1) }
    await Service.DriverService.updateData(getDriverCriteria,ratingDataToSet,{new:true});
...
let submitRestaurantRating = async (payloadData,customerData,orderData)=>{
  try{
    ...
    let dataToSet = {
      "restaurantRatingByCustomer.ratingGivenAt" : new Date(),"restaurantRatingByCustomer.rating":payloadData.rating,"restaurantRatingByCustomer.comment":payloadData.comment ,
    ...
    let queryResult= await Promise.all([
      Service.OrderService.updateData(criteria,dataToSet,{new:true}),
      Service.RestaurantService.getData(getRestaurantCriteria,{rating:1},options)
    ])
    let updateOrderRating  = queryResult[0]; 
    let restaurantDetails = queryResult[1];
    let calculateAvarageRating = (restaurantDetails[0].rating.averageRating != 0) ? (restaurantDetails[0].rating.totalRating+payloadData.rating)/(restaurantDetails[0].rating.noOfTimesRated+1) : payloadData.rating;
    let ratingDataToSet = {$inc:{"rating.totalRating":payloadData.rating,"rating.noOfTimesRated":1},"rating.averageRating":calculateAvarageRating.toFixed(1)}
    await Service.RestaurantService.updateData(getRestaurantCriteria,ratingDataToSet,{new:true});
    return {updateOrderRating};
  }catch(err){
    throw err;
  }
...
let getNotificationList = async (payload,UserData)=>{
  try{
    let  criteria = {userId: UserData._id,userType:APP_CONSTANTS.USER_ROLES.CUSTOMER}
    let options = { 
      skip:payload.skip,
      limit:payload.limit, 
...
let getPromoCode = async (payloadData,UserData)=> {
  ...
  try{
    ...
    let queryResult1= await Promise.all([
      Service.PromoCodeService.aggregateQuery([aggProjection,{$match:{count:0}}]),
