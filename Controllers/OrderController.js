...
let createOrder= async (payloadData,UserData)=> { console.log("payloadData.pricePaidByCustomer",payloadData.pricePaidByCustomer)
  let dishesId=[],menuId=[],toppingIdAarry=[];
  let orderDeliveryTime,promoCodeIsValidOrNot=false,promoCodeData={},promoCodeDetails={};
  
  if(payloadData.paymentType==PAYMENT_TYPES.RAZORPAY && payloadData.razorpayId=="" ){
    throw STATUS_MSG.ERROR.RAZORPAY_ID_IS_REQUIRED;
  }
  if(payloadData.isScheduleOrder){
    if(!payloadData.scheduleOrderDate || typeof(payloadData.scheduleOrderDate)==undefined){
      throw STATUS_MSG.ERROR.SCHEDULE_ORDER_DATE_IS_REQUIRED;
    }
    let scheduleOrderFutureDate = Moment().add(2,'day').format('YYYY-MM-DD HH:mm');
    orderDeliveryTime = payloadData.scheduleOrderDate; 
    if(new Date(scheduleOrderFutureDate).getTime()>new Date(orderDeliveryTime).getTime()){ 
  ...
  try{
    if(payloadData.paymentType==PAYMENT_TYPES.RAZORPAY && payloadData.razorpayId!="" ){
      let captureRazorpayPayment = await RazorpayPaymentsController.capturePayment(payloadData.razorpayId,payloadData.price,payloadData.currency || ""); //console.log("captureRazorpayPayment",captureRazorpayPayment.transactionRespons);
    ...
    let queryGetResult= await Promise.all([
      Service.CustomerAddressService.getData(addrCriteria,projection,{lean:true}),
      Service.RestaurantService.getData(addrCriteriaForRestaurant,projectionForRestaurant,{lean:true}),
    ...
    let pointExistsInPolygonOrNot = await Service.RestaurantAreaService.getData(polygonCriteria,{},{lean:true});
    ...
    let queryResult =[]
    if(payloadData.paymentType==PAYMENT_TYPES.RAZORPAY && payloadData.razorpayId!="" ){ //console.log("heree==106");
      captureRazorpayPayment.transactionType = TRANSACTION_TYPES.ORDER_CREATE_RAZORPAY;
      captureRazorpayPayment.orderId = insertOrder._id;
      queryResult= await Promise.all([
        Service.DishesService.getData(disheCriteria,{__v:0},{lean:true}),
        Service.ToppingItemService.getData(tCriteria,{__v:0},{lean:true}),
        Service.TransactionService.InsertData(captureRazorpayPayment)
      ]);
      
    }else{  //console.log("heree==115",tCriteria);
      queryResult= await Promise.all([
        Service.DishesService.getData(disheCriteria,{__v:0},{lean:true}),
        Service.ToppingItemService.getData(tCriteria,{__v:0},{lean:true}),
      ]);
    ...
    let tempDataArray=[],totalPrice=0
    let emailDataHtml=[];
    disheDetails.forEach((element)=>{
      let tempData={},itemTotalPrice=0;
    ...
    let options = {}
    let insertOrderDetail = await  Service.OrderDetailService.InsertMany(tempDataArray);

    let OrderDetail=[]
    insertOrderDetail.forEach((element)=>{
      OrderDetail.push(element._id)
    });
    let dataToSet = {pricePaidByCustomer:payloadData.pricePaidByCustomer,
      orderDetail:OrderDetail};
     
    let calculateAllCharge = await Helper.OrderHelper.calculateAllCharge(restaurantData,addrDetails,payloadData.totalPrice,payloadData.totalTax,UserData._id,payloadData.promoCode || null);
    ...
    dataToSet.adminGstOnCommission   = calculateAllCharge.calculateAdminGstPercentage || 0
    
    // if(payloadData.promoCode && dataToSet.promoCodeId!=null){
    //   dataToSet.promoCodeId=promoCodeId;
    // }
    console.log("\n\n\n promoCode==266",payloadData.promoCode);
    if(payloadData.promoCode && typeof payloadData.promoCode!=undefined && payloadData.promoCode!=null){
      
      promoCodeData =  await Helper.OrderHelper.checkPromoCode(payloadData.promoCode,payloadData.restaurantId,UserData._id,payloadData.totalPrice,payloadData.itemTotalPrice,customerDeliveryCharge,payloadData.totalTax);
    ...
    await Promise.all([
      Service.OrderService.updateMultipleDocuments({_id:insertOrder._id}, dataToSet, options),
      Service.RestaurantService.updateMultipleDocuments({_id:payloadData.restaurantId}, {$inc:{totalOrder:1}}, {lean:true})
    ])
  ...
  return {orderId:insertOrder._id,
    orderAuoIncrement:insertOrder.orderAuoIncrement,
    OrderDetail
  } 
...
let OrderDetail = async (payload,UserData)=>{ 
  try{
    ...
    let nestedModel = [
      {
        ...
        model: 'topping',
        select: '_id toppingTitleId toppingName price isVegetarian toppingAutoIncrementId subToppingId isChildExists',
        options: {lean: true}
      },
    ] 
    ...
    let projection={};
    let queryResult= await Promise.all([
      Service.DAOService.getDataDeepPopulateFixed(Models.OrderDetailTable,criteria,{__v:0},options,collectionOption2,nestedModel),
...
let customerOrderDetail = async (payload,UserData)=>{ 
  try{
    ...
    let projection={restaurantId:1,address:1,totalPrice:1,orderAuoIncrement:1,pricePaidByCustomer:1,
      orderAuoIncrement:1,paymentType:1,orderType:1,driverId:1,status:1,createdAt:1,isdriverAssigned:1,driverCommission:1,itemTotalPrice:1,totalTax:1,pricePaidByCustomer:1,dicountApplied:1}
    let queryResult= await Promise.all([
      Service.DAOService.getDataDeepPopulateFixed(Models.OrderDetailTable,criteria,{__v:0},options,collectionOption2,nestedModel),
      Service.DAOService.getDataWithReferenceFixed(Models.OrderTable,criteriaOrder,projection,options,collectionOptions),
    ]);
    ...
    for(let element of queryResult[0]){
      for(let innerElement of element.ToppingData){
        ...
        if(innerElement.toppingId && innerElement.toppingId.toppingTitleId){
          gettitle = await Service.ToppingTitleService.getData({_id:innerElement.toppingId.toppingTitleId},{title:1},{lean:true});
...
let getRestaurantOrderList = async (payload,UserData)=>{ 
  try{
    ...
    if(payload.isScheduleOrder){
      ...
      criteria.$or=[{isScheduleOrder:false},{isScheduleOrder:{$exists:false}}]
    }; 
    ...
    let scheduleOrder =  {restaurantId: UserData._id,isScheduleOrder:true,
    ...
    let queryResult= await Promise.all([
      ...
      Service.OrderService.countData({restaurantId: UserData._id,status:ORDER_STATUS.ACCEPTED}),
      Service.OrderService.countData({restaurantId: UserData._id,status:ORDER_STATUS.COOKED}),
      Service.OrderService.countData({restaurantId: UserData._id,status:ORDER_STATUS.PICKED_BY_RIDER}),
      Service.OrderService.countData({restaurantId: UserData._id,status:ORDER_STATUS.DELIVERED_BY_RIDER}),
      Service.OrderService.countData({restaurantId: UserData._id,status:ORDER_STATUS.CANCELLED_BY_CUSTOMER}),
...
let driverNewOrderList = async (payload,UserData)=>{ 
  try{
    ...
    let  criteria = {driverId: UserData._id,status:{
      $nin:[ ORDER_STATUS.CANCELLED_BY_RESTAURANT,
    ...
    let collectionOptions =[
      {
        path: 'restaurantId',
        model: 'restaurant',
        select: '_id restaurantName logo restaurantAutoIncrementId vendorFullAddress state city country',
        options: {lean: true}
      }, 
      {
        path: 'driverId',
