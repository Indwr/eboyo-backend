        ...
        if (element.restaurantId) {
          ...
          if (driverList.length > 0) {
            let requestData = {
              ...
              status: APP_CONSTANTS.ORDER_STATUS.DRIVER_REQUEST_SEND,
            };
            if (driverList[0].deviceToken) {
              ...
              UniversalFunctions.sendNotificationUsingFCM(
                createObjectForNotification
              );
            }
    ...
    return {
      ...
      allOrderList:
        allOrderList.length > 0
          ? JSON.parse(JSON.stringify(allOrderList))
          : allOrderList,
    };
  } catch (err) {
    //console.log("err",err);
...
let getAvailableDriver = async (Data) => {
  console.log("getAvailableDriver==init");
  console.table(Data);
  try {
    let sendRequestList = [],
      sendRequestResult = [];
    if (Data.driverRequestSendCount > 0) {
      ...
      sendRequestList = await Service.DriverOrderRequestService.getData(
        criteriaD,
        { driverId: 1 },
        { lean: true }
      );
      sendRequestList.forEach((elementD) => {
        //sendRequestResult.push(elementD.driverId.toString());
