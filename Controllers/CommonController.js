...
let getCatgory = async (payloadData, UserData) => {
  console.log("getCatgory==init");
  try {
...
let getRestaurant = async (payloadData, UserData) => {
  ...
  try {
    let nearByRestaurant = await checkPointExistInPolygon(
      payloadData,
      UserData
    );
    ...
    if (nearByRestaurant.length > 0) {
      ...
      if (restaurantWorkingId.length > 0) {
        ...
        let finalData = [];
        for (let element of queryResult[1]) {
          //console.log("element",element);
          let tempData = element;
          let menuC = {
            availabilityStatus: true,
            isDeleted: false,
            restaurant: element._id,
          };
          let promocodeC = {
          ...
          let queryRes = await Promise.all([
            Service.RestaurantMenuService.getData(
              menuC,
              { menuName: 1 },
              { lean: true, skip: 0, limit: 5 }
            ),
            //Service.RestaurantWorkingTimeService.getData(workingC, {__v:0,restaurant:0}, {lean:true}),
            Service.PromoCodeService.getData(
              promocodeC,
              { __v: 0, restaurant: 0, customer: 0 },
              { lean: true, skip: 0, limit: 2 }
            ),
          ]);
...
let getNearByRestaurant = async (payloadData, UserData) => {
  ...
  try {
    ...
    let checkPointExistsInPolygonOrNot =
      await Service.RestaurantService.aggregateQuery([criteria]);
...
let checkPointExistInPolygon = async (payloadData, UserData) => {
  ...
  try {
    ...
    let checkPointExistsInPolygonOrNot =
      await Service.RestaurantAreaService.getData(criteria, {}, { lean: true });
    if (checkPointExistsInPolygonOrNot.length > 0) {
      for (let element of checkPointExistsInPolygonOrNot) {
        for (let innerElement of element.restaurant) {
          restaurantId.push(innerElement);
...
let checkRestaurantWorking = async (nearByRestaurant) => {
  let criteria2 = {
    restaurant: { $in: nearByRestaurant },
    //"dayName": payloadData.dayName
  };
  let restaurantId = [];
  try {
    let restaurant = await Service.RestaurantWorkingTimeService.getData(
      criteria2,
      {},
      { lean: true }
    );
    for (let element of restaurant) {
      //console.log("checkRestaurantWorking===element==182",element);
  ...
  let collectionOptions = [
    {
      ...
      model: "toppingtitle",
      select:
        "_id title isChildExists isVegetarian toppingTitleAutoIncrementId maxItemSelection necessaryItemSelection toppingId",
      options: { lean: true },
    },
  ];
  let nestedModel = [
    {
      ...
      model: "topping",
      select:
        "_id title toppingName price isVegetarian toppingAutoIncrementId subToppingId isChildExists",
      options: { lean: true },
    },
  ];
  try {
    ...
    if (countMenu > 0) {
      for (let i = 0; i < countMenu; i++) {
        ...
        let dishesData = await Service.DAOService.getDataDeepPopulateFixed(
          Models.Dishes,
          disheCriteria,
...
let getRestaurantSubToppings = async (payloadData, UserData) => {
  try {
    let criteria = {
      menuId: payloadData.menuId,
      toppingId: payloadData.toppingId,
      isDeleted: false,
    };
    let projection = {
      createdAt: 0,
...
const sendEmail = async (payloadData) => {
  try {
    ...
    let sendStr = emailTemplate
      ...
      .replace("{{logo}}", logo).payloadData.replaceString;
...
let getCityList = async (payloadData, UserData) => {
  try {
    ...
    let queryResult = await Promise.all([
      ...
      Service.CityService.aggregateQuery([
        match,
        groupBy,
        { $sort: { _id: 1 } },
        skip,
...
let getRestaurantTiming = async (payloadData, UserData) => {
  try {
    ...
    queryResult = await Service.RestaurantWorkingTimeService.getData(
      criteria,
      {},
      options
    );
...
let allFaq = async (payloadData, UserData) => {
    ...
    let collectionOptions = [
      {
        ...
        model: "menu",
        select:
          "_id menuName menuImage availabilityStatus menuAutoIncrementId ",
        options: { lean: true },
      },
      {
        path: "toppingTitleId",
        model: "toppingtitle",
        select:
          "_id title isChildExists isVegetarian toppingTitleAutoIncrementId ",
        options: { lean: true },
      },
    ];
...
let allBanner = async (payloadData, UserData) => {
  console.log("getFaq==init");
  try {
    let promoCodeData = [];
    ...
    let queryResult = await Promise.all([
      ...
      Service.PromoCodeService.getData({}, projectionForPromoCode, {
        lean: true,
      }),
      Service.CuisineService.getData(
        { isEnabled: true, isDeleted: false },
        projectionForCuisine,
        { lean: true }
      ),
    ]);
    let totalCount = queryResult[0] || [];
  ...
  try {
    ...
    if (countMenu > 0) {
      ...
      for (let element of restaurantMenuData) {
        ...
        let dishesData = await Service.DishesService.getData(
          disheCriteria,
          projectionD,
          optionsD
        );
...
let dishesTitelAndTopping = async (payloadData, UserData) => {
  console.log("dishesTitelAndTopping====init");
  let collectionOptions = [
    {
      path: "toppingId",
      model: "topping",
      select:
        "_id title toppingName price isVegetarian toppingAutoIncrementId  isChildExists isDefault subToppingId",
      options: { lean: true },
    },
  ];
  let nestedModel = [
    {
      ...
      select:
        "_id title toppingName price isVegetarian toppingAutoIncrementId subToppingId isDefault isChildExists",
      options: { lean: true },
    },
  ];
  try {
...
let generatePdf = async (payloadData) => {
  try {
    let templatepath = Path.join(
      __dirname,
      "../pdfTemplates/" + payloadData.pdfTemplate
    );
    ...
    var document = {
      ...
      data: {
        pdfData: pdfData,
        headers: headers,
        logo: APP_CONSTANTS.APP_DETAILS.LOGO,
      },
