...
//  const Controller = require("../../Controllers");
 ...
 let allBank = async (payloadData,UserData)=> { console.log("getFaq==init");
   try {
...
let getBank = async (payloadData,UserData)=> { console.log("getFaq==init");
   try {
    let returnData;
    let criteria = {"isDeleted" : false ,userId:payloadData.userId,userType:payloadData.userType }
    let projection = {__v:0,bannerAuoIncrement:0}
    let options = {lean:true}
    let queryResult = await Service.BankDetailService.getData(criteria, projection, options);
...
let editBank = async (payloadData,userData) => {
  ...
  let criteria = {userId: payloadData.userId,"isDeleted" : false,userType:payloadData.userType};
  ...
  try {
    ...
    let updateCriteria = { userId: bankData[0][0].userId,userType:payloadData.userType };
    ...
    let finalData = await Service.BankDetailService.updateData(updateCriteria,dataToSet,{ new: true });
...
let deleteBank = async (payloadData,userData) => {
  ...
  let projection = {__v: 0};
  try {
    let bankData = await Service.BankDetailService.getData(criteria, projection, {
      lean: true,
    });
    ...
    let finalData = await Service.BankDetailService.updateData(updateCriteria,
      dataToSet,{ new: true });
    return { bankData: finalData };
  } catch (err) {
    throw err;
