...
let mapRestaurant = async (payloadData, UserData)=>{   
  try{
    let result= await Promise.all([ 
      Service.RestaurantAreaService.getData({restaurant:{$in:[payloadData.restaurantId]}},{},{}),
      Service.RestaurantAreaService.getData({_id:payloadData.locationId},{},{}) 
    ]);
    ...
    if(existInPolygun.length>0 && payloadData.locationId!=existInPolygun[0]._id){
      throw STATUS_MSG.ERROR.RESTAURANT_ALREADY_MAPED_OTHER_POLYGON;
    }
    let dataToSet = {
      $addToSet:{restaurant: payloadData.restaurantId},
    }; 
    servingLocation  = await Service.RestaurantAreaService.updateMultipleDocuments({_id:payloadData.locationId},dataToSet,{new:true,lean:true});
    return {};
  }catch(err){
    throw err;
  }
  ...
  try{
    ...
    let projection = { 
      $project:{
        ...
        "result._id":1,
        "result.logo":1,"result.email":1,"result.restaurantName":1,
        "result.restaurantName":1,"result.vendorFullAddress":1,
    ...
    let queryResult = await Promise.all([
      Service.RestaurantAreaService.aggregateQuery([match,lookup,projection,unwind,count]),
      Service.RestaurantAreaService.aggregateQuery([match,lookup,projection,unwind,{$sort:{"esult.restaurantName":1}},skip,limit]),
      Service.RestaurantAreaService.getData(criteria,{restaurant:0,__v:0},options)
    ])
...
const getLocationRestaurant = async (payloadData,UserData)=> {
  try{
    ...
    let count = {$count: "totalDocument"};
    let queryResult = await Promise.all([
      Service.RestaurantAreaService.aggregateQuery([lookup,project,unwind,count]),
      Service.RestaurantAreaService.aggregateQuery([lookup,project,unwind,{$sort:{"esult.restaurantName":1}},skip,limit])
    ])
    let totalCount= (queryResult[0].length>0) ? queryResult[0][0].totalDocument:0;
    let locationRestaurantlist  = queryResult[1]; 
