...
let create = async (payloadData, UserData) => {
  ...
  try {
    ...
    let queryResult = await Promise.all([
      ...
      Service.CategoryService.getData(
        { _id: { $in: payloadData.category } },
        { _id: 1 },
        {}
      ),
    ]);
    ...
    payloadData.encryptedPassword =
      UniversalFunctions.encryptedPassword(generatePassword);
    ...
    for (let element of DEFAULT_RESTAURANT_WORKING_TIME) {
      ...
      let saveW = await Service.RestaurantWorkingTimeService.InsertData(
        element
      );
    }
  ...
  try {
    ...
    let result = await Promise.all([
      ...
      Service.RestaurantAreaService.getData(
        locationC,
        { deliveryServiceArea: 0, restaurant: 0 },
        {}
      ),
      Service.BankDetailService.getData(
        { userId: payloadData.restaurantId, userType: "Restaurant" },
        {},
        { lean: true }
      ),
    ]);
...
let editRestaurant = async (payloadData, UserData) => {
  try {
    ...
    //console.log("data",data);
    restaurantData = await Service.RestaurantService.updateData(
      criteria,
      dataToSet,
      { new: true, lean: true }
    );
