...
let getCuisine = async (payloadData, UserData) => {
...
let getSingleCuisine = async (payloadData, UserData) => {
  console.log("getFaq==init");
  try {
    let criteria = { isDeleted: false, isEnabled: true, _id: payloadData._id };
    let projection = { createdAt: 0, updatedAt: 0, __v: 0 };
    let options = { lean: true };
    let queryResult = await Service.CuisineService.getData(
      criteria,
      projection,
      options
    );
...
let editCuisine = async (payloadData, userData) => {
  ...
  try {
    let cuisineData = await Service.CuisineService.getData(
      criteria,
      projection,
      { lean: true }
    );
    ...
    let finalData = await Service.CuisineService.updateData(
      updateCriteria,
      dataToSet,
      { new: true }
    );
