...
const addMenu = async (payloadData, UserData) => {
  try {
    ...
    let getMenudata = await Service.AdminMenuService.getData(
      criteria2,
      {},
      options2
    );
    ...
    if (payloadData.menuId) {
      if (typeof payloadData.menuImage != "undefined") {
        if (
          payloadData.menuImage["_data"].length > DOCUMENT_FILE_SIZE.IMAGE_SIZE
        ) {
      ...
      let menuData = await Service.AdminMenuService.updateData(
        criteria,
        payloadData,
        { new: true, lean: true }
      );
...
let deleteMenu = async (payload, UserData) => {
  try {
    ...
    await Service.AdminMenuService.updateMultipleDocuments(
      criteria,
      dataToSet,
      options
    );
...
let updateImage = async (payloadData, UserData) => {
  try {
    ...
    let menuData = await Service.AdminMenuService.getData(
      criteria,
      {},
      { lean: true }
    );
    if (menuData) {
      ...
      payloadData.menuImage = imageFile[0];
      updateMenuImage = await Service.AdminMenuService.updateData(
        criteria,
        payloadData,
        { lean: true, new: true }
      );
