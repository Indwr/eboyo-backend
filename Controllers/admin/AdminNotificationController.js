 ...
 */
 ...
 let allNotification = async (payloadData,UserData)=> { console.log("getFaq==init");
   try {
    let criteria = {}
...
let addNotification = async (payloadData,UserData) => {
  try {
    ...
    let createNotificationText = await Service.NotificationTextService.InsertData(data);
...
let editNotification = async (payloadData,userData) => {
  ...
  let projection = {__v: 0};
  try {
    let notificationTextData = await Service.NotificationTextService.getData(criteria, projection, {
      lean: true,
    });
