...
let create = async (payloadData, UserData) => {
  try {
    ...
    if (
      payloadData.adminPercentage == 0 &&
      payloadData.restaurantPercentage == 0
    ) {
      throw STATUS_MSG.ERROR.ADMIN_RESTAURANT_PERCENTAGE_CAN_NOT_ZERO;
    }
    let totalPercentage =
      payloadData.adminPercentage + payloadData.restaurantPercentage;
...
let updateStatus = async (payloadData, UserData) => {
  try {
    ...
    let createPromo = await Service.PromoCodeService.updateData(
      { _id: payloadData.promoCodeId },
...
let updateImage = async (payloadData, UserData) => {
  try {
    ...
    if (promoCodeData) {
      ...
      payloadData.image = imageFile[0];
      updatePromoCodeImage = await Service.PromoCodeService.updateData(
        criteria,
        payloadData,
        { lean: true, new: true }
      );
