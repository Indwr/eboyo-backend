  ...
  let getOrderRequestList = async (payloadData, UserData) => {
    ...
    try {
      ...
      let OrderPromise = await Promise.all([
        ...
        Service.DAOService.getDataWithReferenceFixed(Models.DriverOrderRequestTable,criteria,{__v:0},{},collectionOptions),
      ])
      let totalOrderQ = OrderPromise[0]
      let orderData =OrderPromise[1]; 
      return {
  ...
  let getRestaurantOrderList = async (payload,UserData)=>{ 
    try{
      ...
      let scheduleOrder =  {restaurantId: UserData._id,isScheduleOrder:true,
        status:{$in :[APP_CONSTANTS.ORDER_STATUS.PENDING,APP_CONSTANTS.ORDER_STATUS.ACCEPTED ]
        }
      }
      let queryResult= await Promise.all([
        ...
        Service.OrderService.countData({status:APP_CONSTANTS.ORDER_STATUS.ACCEPTED}),
        Service.OrderService.countData({status:APP_CONSTANTS.ORDER_STATUS.COOKED}),
        Service.OrderService.countData({status:APP_CONSTANTS.ORDER_STATUS.PICKED_BY_RIDER}),
        Service.OrderService.countData({status:APP_CONSTANTS.ORDER_STATUS.DELIVERED_BY_RIDER}),
        Service.OrderService.countData({status:APP_CONSTANTS.ORDER_STATUS.CANCELLED_BY_CUSTOMER}),
        Service.OrderService.countData({status:APP_CONSTANTS.ORDER_STATUS.CANCELLED_BY_RESTAURANT }),
    ...
    try{
      ...
      let geoNear = {
        $geoNear: {
          ...
          query: { Isverified:true,Islogin:true,IsBusy:false,IsBlocked:false },
      ...
      let aggregateQuery= [geoNear,{$project:{
        _id:1,driverNumber:1,currentLocation:1,
        profilePicURL:1,rating:1,fullName:1,email:1,phoneNo:1}},
        {$skip:payload.skip},{$limit:payload.limit}]
      let coutAggregateQuer=[geoNear,{
        $count: "totalRecord"
      }]  
