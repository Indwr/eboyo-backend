...
const login = async (payloadData) => {
  ...
  try {
    let userData = await Service.RestaurantService.getData(
      criteria,
      projection,
      { lean: true }
    ...
    let accessToken = await UniversalFunctions.generateAuthToken({
      ...
      role: APP_CONSTANTS.USER_ROLES.RESTAURANT,
    ...
    let finalData = await Service.RestaurantService.updateData(
      updateCriteria,
      dataToSet,
      { new: true }
    ); //console.log("==finalData==",finalData);
    let restaurantDataInDetail =
    ...
    let finalresult = await setRestaurantResponse(
      JSON.parse(JSON.stringify(restaurantDataInDetail[0]))
    );
...
let forgotPassword = async (payloadData, UserData) => {
  ...
  try {
    ...
    let updatePassWord = await Service.RestaurantService.updateData(
      criteria,
      dataToSet,
      { new: true }
    );
...
let resetPassword = async (payloadData) => {
  ...
  try {
    let getRestaurantDetail = await Service.RestaurantService.getData(
      criteria,
      {},
      { lean: true }
    );
...
const editRestaurantOnlieStatus = async (payloadData, UserData) => {
  try {
    ...
    let data = await Service.RestaurantService.updateMultipleDocuments(
      { _id: UserData._id },
      dataToSet,
      { new: true }
    );
...
let addDeliveryServiceArea = async (payloadData, UserData) => {
  ...
  try {
    ...
    servingLocation = await Service.RestaurantService.updateMultipleDocuments(
      { _id: UserData._id },
      dataToSet,
      { new: true, lean: true }
    );
...
let orderAccepted = async (payload, UserData) => {
  try {
    ...
    let orderData = await Service.OrderService.getData(
      criteria,
      { address: 0, __v: 0 },
      {}
    );
    ...
    let notificationData = {
      ...
      message:
        NOTIFICATION_MESSAGE.ORDER_ACCEPTED +
        "(#" +
        orderData[0].orderAuoIncrement +
        ")",
    };
    let queryResult = await Promise.all([
...
let updateOrderStatus = async (payload, UserData) => {
  try {
    ...
    if (
      customerData[0].firstName != undefined &&
      customerData[0].firstName != ""
    ) {
      firstName = customerData[0].firstName;
    }
    if (
      customerData[0].lastName != undefined &&
...
let updateProfile = async (payloadData, UserData) => {
  ...
  try {
    ...
    let queryResult = await Promise.all([
      UniversalFunctions.uploadFilesWithCloudinary(
    ...
    restaurantData = await Service.RestaurantService.updateMultipleDocuments(
      criteria,
      dataToSet,
      { lean: true }
    );
    ...
    let finalRestaurantData = await Service.RestaurantService.getData(
      criteria,
      lastProjection,
      { lean: true }
    );
...
let getTransactionGroupByDate = async (payloadData, UserData) => {
  console.log("getFaq==init");
  try {
    let match = {
      $match: {
    ...
    let queryResult = await Promise.all([
      Service.TransactionService.aggregateQuery([
        match,
        groupBy,
        { $sort: { _id: 1 } },
        skip,
...
let submitWithdraw = async (payloadData, UserData) => {
  try {
    ...
    let withdrawTransaction = {
      restaurantId: UserData._id,
      transactionType:
        APP_CONSTANTS.TRANSACTION_TYPES.SUBMIT_WITHDRAW_FROM_RESTAURANTS,
  ...
  try {
    ...
    for (let element of payloadData.workingDays) {
      ...
      if (payloadData.isUpdate) {
        ...
        let updateData = await Service.RestaurantWorkingTimeService.updateData(
          criteria,
          saveData,
          { new: true }
        );
...
let getWorkingTime = async (payloadData, UserData) => {
  try {
    ...
    let finalData = await Service.RestaurantWorkingTimeService.getData(
      criteria,
      { __v: 0 },
      { lean: true }
    );
...
let updateWorkingStatusOfDay = async (payloadData, UserData) => {
  try {
    ...
    let dataToSet = {
      isRestaurantOpenOrNot: payloadData.isRestaurantOpenOrNot,
    };
    let updateData = await Service.RestaurantWorkingTimeService.updateData(
      criteria,
      dataToSet,
...
let getWalletAmount = async (payloadData, UserData) => {
  try {
    ...
    let walletAmount = await Service.TransactionService.getData(
      criteria,
      {},
      { lean: true }
    );
    ...
    return {
      ...
      bankDetails:
        UserData.razorpayXData != undefined
          ? UserData.razorpayXData.bank_account
          : "",
    };
  } catch (err) {
    throw err;
