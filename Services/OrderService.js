...
var InsertData = function (objToSave) {    
    return new Promise((resolve, reject) => {
        new OrderTable(objToSave).save().then(data => {
            return resolve(data);
        }).catch(err => { 
...
var updateData = function (criteria, dataToSet, options) {
    return new Promise((resolve, reject) => {
        OrderTable.findOneAndUpdate(criteria, dataToSet, options).then(data => {
...
let updateMultipleDocuments = (criteria, dataToSet, options)=>{
    return new Promise((resolve, reject) => {
        OrderTable.updateMany(criteria, dataToSet, options).then(data => {
            return resolve(data);
        }).catch(err => {
            if (err.code == 11000 || 11001 === err.code) {
                if (err.errmsg.indexOf('categoryName_1_restaurant_1') > -1) return reject(STATUS_MSG.ERROR.CATEGORY_NAME_EXISTS);
...
let aggregateQuery = async(groupData)=>{
    try{
      let data = await OrderTable.aggregate(groupData);
      return  data
    }catch(error){
        throw error;
    }
...
let countData = async (condition)=>{    
    try{
        let count=  await OrderTable.countDocuments(condition);
        return count
    }catch(error){
        throw error;
    }
...
let dropIndex =  (IndexName)=>{
    return new Promise((resolve, reject) => {
        OrderTable.collection.dropIndex(IndexName).then(() => {
            return resolve();
        }).catch(err => {
            return reject(err);             
        })
