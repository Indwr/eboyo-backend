...
const tableName        = Models.Toppings
    ...
    return new Promise((resolve, reject) => {
        new tableName(objToSave).save().then(data => {
            ...
            if (err.code == 11000 || 11001 === err.code) { 
                if (err.errmsg.indexOf('restaurant_1_menuId_1_dishId_1_titleId_1_toppingName_1') > -1) return reject(ERROR.TOPPING_ITEM_ALREADY_EXISTS); 
                return reject(err);
            } else {
                return reject(err);
            }
