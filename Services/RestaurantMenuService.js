...
var InsertData = function (objToSave) {  //console.log("===========InsertData========objToSave========",objToSave);
    return new Promise((resolve, reject) => {
        new RestaurantMenu(objToSave).save().then(data => {
            return resolve(data);
        }).catch(err => { 
...
var updateData = function (criteria, dataToSet, options) {
    return new Promise((resolve, reject) => {
        RestaurantMenu.findOneAndUpdate(criteria, dataToSet, options).then(data => {
...
let updateMultipleDocuments = function (criteria, dataToSet, options) {
    return new Promise((resolve, reject) => {
        RestaurantMenu.updateMany(criteria, dataToSet, options).then(data => {
            return resolve(data);
        }).catch(err => {
            if (err.code == 11000 || 11001 === err.code) {
                if (err.errmsg.indexOf('categoryName_1_restaurant_1') > -1) return reject(STATUS_MSG.ERROR.CATEGORY_NAME_EXISTS);
...
let countData = async (condition)=>{    
    try{
        let count=  await RestaurantMenu.countDocuments(condition);
        return count
    }catch(error){
        throw error;
    }
...
let dropIndex =  (IndexName)=>{
    return new Promise((resolve, reject) => {
        RestaurantMenu.collection.dropIndex(IndexName).then(() => {
            return resolve();
        }).catch(err => {
            return reject(err);             
        })
...
let aggregateQuery = async(groupData)=>{
    try{
      let data = await RestaurantMenu.aggregate(groupData);
      return  data
    }catch(error){
        throw error;
    }
