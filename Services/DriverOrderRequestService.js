...
const tableName        = Models.DriverOrderRequestTable
    ...
    return new Promise((resolve, reject) => {
        new tableName(objToSave).save().then(data => {
            ...
            if (err.code == 11000 || 11001 === err.code) { 
                if (err.errmsg.indexOf('orderId_1_driverId_1') > -1) return reject(STATUS_MSG.ERROR.ALREADY_ORDER_REQUEST_SEND_DRIVER); 
                return reject(err);
            } else {
                return reject(err);
            }
