...
const tableName        = Models.BankDetailsTable
    ...
    return new Promise((resolve, reject) => {
        new tableName(objToSave).save().then(data => {
            ...
            if (err.code == 11000 || 11001 === err.code) { 
                // if (err.errmsg.indexOf('bannerName_1') > -1) return reject(STATUS_MSG.ERROR.BANNER_NAME_ALREADY_EXISTS); 
                return reject(err);
            } else {
                return reject(err);
            }
...
var updateData = function (criteria, dataToSet, options) {
    return new Promise((resolve, reject) => {
        console.log('updateBank ' , criteria , dataToSet , options)
