...
let calculateAllCharge = async (restaurantData,addrDetails,totalPrice,restaurantTotalTax,customerId,promoCode=null)=>{
  try{
    let driverCommission=0,finalDistance=0,calculateAdminCommission=0,calculateAdminGstPercentage=0;
    let customerDeliveryCharge=0,riderDeliveryCharge=0;
    ...
    let finalData= await calculateDeliveryCharge(cityId,restaurantLocation,customerLatLong[1],customerLatLong[0]);
    ...
    //including Tax 
    console.log("totalPrice",totalPrice,restaurantTotalTax,customerDeliveryCharge)
    let itemTotalPrice =  totalPrice-restaurantTotalTax-customerDeliveryCharge;
    console.log("itemTotalPrice",itemTotalPrice,restaurantTotalTax,customerDeliveryCharge)
    ...
    if(calculateAdminCommission>0){
      calculateAdminGstPercentage =(calculateAdminCommission*restaurantData.adminGstPercentage)/100;
      calculateAdminGstPercentage= parseFloat((calculateAdminGstPercentage.toFixed(2)))
    }
    let promoCodeId=null;
    if(promoCode!==null){
      let promoCodeData = await checkPromoCode(promoCode,restaurantData._id,customerId,totalPrice);
    ...
    let tempConsole={itemTotalPrice,customerDeliveryCharge,riderDeliveryCharge,
      driverCommission:riderDeliveryCharge,finalDistance,calculateAdminCommission,restaurantCommission,calculateAdminGstPercentage,promoCodeId
    }; //console.log("tempConsole",tempConsole);
    return {itemTotalPrice,customerDeliveryCharge,riderDeliveryCharge,
      driverCommission:riderDeliveryCharge,finalDistance,calculateAdminCommission,restaurantCommission,calculateAdminGstPercentage,promoCodeId
    }
  }catch(error){
    throw error;
...
let calculateDistance = async (restaurantData,addrDetails)=>{
  try{
    let driverCommission=0,finalDistance=0;
    let cityCriteria= {_id:restaurantData.cityId}

    let cityData= await Service.CityService.getData(cityCriteria,{__v:0,password:0,accessToken:0,deliveryServiceArea:0}, {lean:true});

    if(cityData.length>0){
      let restaurantLocation = restaurantData.location.coordinates;
      let addressLocation  = addrDetails[0].location.coordinates;
      finalDistance =  await CalcDistanceBetweenCoordinates.calculateDistance(restaurantLocation[1],restaurantLocation[0],addressLocation[1],addressLocation[0]);
    }
    ...
    if(cityData.length>0 && finalDistance>0){
      ...
      if(cityData[0].deliveryCharge.minimumDistance > finalDistance){
        ...
        driverCommission=minimumPrice+(finalDistance-minimumDistance)*pricePerkm
      }
    }
    return {
...
let checkPromoCode= async (promoCode,restaurantId,customerId,totalPrice,itemTotalPrice,customerDeliveryCharge=0,totalTax=0)=>{
  
  let finalDiscountAmount=0;
  let itemPrice=itemTotalPrice;  
  console.log("\n\ntotalPrice:",totalPrice,"\nitemPrice:",itemPrice,"\ncustomerDeliveryCharge:",customerDeliveryCharge,"\ntotalTax:",totalTax,"\nitemPrice:",itemPrice,"\n\n\n");
  
  let adminPartPercentage=0,restaurantPartPercentage=0;
  let finalAdminPartAmt=0,finalRestaurantPartAmt=0
  let promoCodeIsValidOrNot = false;
  try{
    
    let promoCodeRestaurantList=[],promoCodeCustomerList=[];
    let options={lean:true};     
    let criteria = {promoCode:promoCode};
    let promoCodeData = await Service.PromoCodeService.getData(criteria,{},options);
    ...
    if(promoCodeIsValidOrNot){ 
      itemPrice = (promoCodeData[0].includeDeliveryCharges==true)? itemPrice+customerDeliveryCharge:itemPrice;
      let maxDiscountAmt= promoCodeData[0].maxDiscountAmt;
      if(promoCodeData[0].discountInPercentage!=0){
        finalDiscountAmount = (itemPrice*promoCodeData[0].discountInPercentage)/100;
        console.log("finalDiscountAmount===161",finalDiscountAmount);
        finalDiscountAmount = parseFloat(finalDiscountAmount.toFixed(2));
        finalDiscountAmount = (finalDiscountAmount>maxDiscountAmt) ? maxDiscountAmt:finalDiscountAmount;
      }else{
      ...
      finalRestaurantPartAmt = (finalDiscountAmount*restaurantPartPercentage)/100;
...
let  calculateDeliveryCharge = async (cityId,restaurantLocation,customerLat,customerLong)=>{
  ...
  try{
    ...
    if(cityData.length>0){
      finalDistance =  await CalcDistanceBetweenCoordinates.calculateDistance(restaurantCoordinates[1],restaurantCoordinates[0],customerLat,customerLong);
    }; //console.log("finalDistance",finalDistance);
    let deliveryCharge=0;
    let customerDeliveryCharge=0,riderDeliveryCharge=0;
    finalDistance =parseFloat(finalDistance);
    if(cityData.length>0){
      let customerMinimumDistance = cityData[0].customerDeliveryCharge.minimumDistance;
      let customerPricePerkm =cityData[0].customerDeliveryCharge.pricePerkm;
      let customerMinimumPrice = cityData[0].customerDeliveryCharge.minimumPrice;
      
      if(finalDistance>customerMinimumDistance){
        customerDeliveryCharge = customerMinimumPrice+(finalDistance-customerMinimumDistance)*customerPricePerkm
      }else{
        customerDeliveryCharge = customerMinimumPrice
      }
      customerDeliveryCharge=parseFloat(customerDeliveryCharge.toFixed(2));
      
      let riderMinimumDistance = cityData[0].riderDeliveryCharge.minimumDistance;
      ...
      if(finalDistance>riderMinimumDistance){
        riderDeliveryCharge = riderMinimumPrice+(finalDistance-riderMinimumDistance)*riderPricePerkm
      }else{
    ...
    return {finalDistance,cityData,riderDeliveryCharge,customerDeliveryCharge};
  }catch(error){
    throw error;
  }
}
