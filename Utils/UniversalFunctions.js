...
let sendMail = async(emailDetail)=> {
  try{
    let emailParams = {
      to:  emailDetail.to || 'anurag.jindal816@gmail.com',
...
let checkDeviceTokenAndDelete = async (deviceData,userType=APP_CONSTANTS.USER_ROLES.CUSTOMER) => {
  try{
    ...
    if(userType==APP_CONSTANTS.USER_ROLES.CUSTOMER){
      ...
      if(getDeviceToken.length>0){  
        ...
        let finalData    = await Service.CustomerService.updateMultipleDocuments(updateCriteria,dataToSet,{new:true,multi:true});
      } 
    }else if(userType==APP_CONSTANTS.USER_ROLES.DRIVER){
      let getDeviceToken    =   await Service.DriverService.getData(criteria,projection,{lean:true});     
      if(getDeviceToken.length>0){ console.log("getDeviceToken==if",getDeviceToken.length);
        let updateCriteria = {_id:getDeviceToken[0]._id}
        let dataToSet  = {$unset:{deviceToken:1}}
        let finalData    = await Service.DriverService.updateMultipleDocuments(updateCriteria,dataToSet,{new:true,multi:true});
      } 
    }    
    return true;
  }catch(err){ //console.log("==========catch=======errrrr====",err);
...
const uploadFiles  = async(document,imagePrefix="catImage_",uniqueId=Date.now(),folderName="restaurant",contentType="")=>{
  try{
    let dataImageArray=[]; 
    if(Array.isArray(document)==true){
      for(var i=0;i<document.length;i++){
        let imageList= await uploadDocumentOnLocalMachine(document[i],{_id:uniqueId},imagePrefix+i);  
        let s3bucketpath = await uploadDocumentOnS3BucketAWSSDK(imageList,folderName,contentType);
        dataImageArray.push({
      ...
      let imageList= await uploadDocumentOnLocalMachine(document,{_id:uniqueId},imagePrefix);
      let s3bucketpath = await uploadDocumentOnS3BucketAWSSDK(imageList,folderName,contentType); //console.log("s3bucketpath",s3bucketpath)
      dataImageArray.push({
        original:s3bucketpath.Location,
        thumbnail:s3bucketpath.Location,
      });
  ...
  }catch(err){ //console.log("uploadFiles===err",err);   
     throw err;
  }
}

const uploadFilesWithCloudinary  = async(document,imagePrefix="catImage_",uniqueId=Date.now(),folderName="restaurant",contentType="")=>{
  try{
    let dataImageArray=[]; 
    if(Array.isArray(document)==true){
      for(var i=0;i<document.length;i++){
        let imageList= await uploadDocumentOnLocalMachine(document[i],{_id:uniqueId},imagePrefix+i);  
        let ImageDate = await cloudinary.uploader.upload(imageList,
        { resource_type: "image"}, 
      ...
      let ImageDate = await cloudinary.uploader.upload(imageList,{ resource_type: "image"}, function(error, result) 
      {});
      dataImageArray.push({
        original:ImageDate.secure_url,
        thumbnail:ImageDate.secure_url,
...
const uploadMultipleFiles  = async(document,imagePrefix="catImage_",uniqueId=Date.now(),folderName="restaurant")=>{
  try{
    let dataImageArray=[]; 
    if(Array.isArray(document)==true){
      for(var i=0;i<document.length;i++){
        ...
        let imageList= await uploadDocumentOnLocalMachine(document[i],{_id:uniqueId},imagePrefix+i+"_");  
        let s3bucketpath = await uploadDocumentOnS3BucketAWSSDK(imageList,folderName,contentType);
        dataImageArray.push({
          original:s3bucketpath.Location,
          thumbnail:s3bucketpath.Location,
...
let uploadDocumentOnS3BucketAWSSDK  = async(ImagePath,folder,contentType)=> { 
  ...
  try{
    let d1 = await promiseT.then((data)=>{  
...
let convertCsvToJson = async(filePath)=>{ console.log("asasdasdsa",filePath);
  try{
...
let generateOrderHtmlAndSendEmail= async (htmlData)=>{
  try{
    ...
    <table width="100%" cellspacing="0" cellpadding="0" style="margin:0;padding:0">
      ...
      rowHtml = rowHtml+`<tr style="margin:0;padding:0">
                        <td style="vertical-align:top;margin:0;padding:10px;font-weight:normal;font-size:13px">`+element.dishName+`</td>
                        <td style="margin:0;text-align:right;padding:10px;font-weight:normal;font-size:13px;padding-right:200px">`+element.quantity+`</td>
                        
                        <td align="right" style="margin:0;padding:10px;font-weight:normal;font-size:13px;padding-right:15px">₹&nbsp;`+element.itemTotalPrice+` </td>
                    </tr>
    ...
    rowHtml=rowHtml+`<tr><td><span><span></span></span></td></tr>`;
    rowHtml=rowHtml+`<tr style="margin:0;padding:0"><td width="80%" scope="row" colspan="2" style="margin:0;padding:10px 0;text-align:right;font-weight:normal;border:0;font-size:13px">Delivery Charges:</td>
    ...
    //back row
    rowHtml=rowHtml+`<tr width="100%"><td><div style="width:100%;clear:both"></div></td><td><div style="min-height:15px;width:100%;clear:both"></div></td><td><div style="min-height:15px;width:100%;clear:both"></div></td></tr>`;
    ...
    let  EmailPayloadData = {
      ...
      subject:'Your Eboyo order summary for order no.'+htmlData.orderAuoIncrement
    };
    let templatepath      = Path.join(__dirname, '../emailTemplates/');
    let fileReadStream    =  templatepath + APP_CONSTANTS.EMAIL_TEMPLATES.ORDER_PLACE
    ...
    let userName = htmlData.customerData.firstName+ ' '+ htmlData.customerData.lastName;
    console.log("EmailPayloadData",EmailPayloadData);

    
    let sendStr = emailTemplate.replace('{{appName}}', appName)
     .replace('{{logo}}',logo)
...
module.exports = {
  generatePassword:generatePassword,
   ...
   sendSMS:sendSMS,
   sendMail:sendMail,
   sendNotificationMultipleDeviceUsingFCM:sendNotificationMultipleDeviceUsingFCM,
   ...
   sendNotificationMultipleDeviceUsingFCM2:sendNotificationMultipleDeviceUsingFCM2,
